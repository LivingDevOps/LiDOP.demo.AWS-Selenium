...
pipeline {
  ...
  stages {
    ...
    stage("Set Build Parameters") {
      steps {
        script {
          currentBuild.displayName = "Destroy_Environment .${BUILD_NUMBER}";
        }
      }
    }
      ...
      steps {
        dir("${WORKSPACE}"){
          withCredentials([string(credentialsId: "${params.AWS_Secret}", variable: 'AWS_SECRET'), string(credentialsId: "${params.AWS_Key}", variable: 'AWS_KEY'), sshUserPrivateKey(credentialsId: "${params.Private_Key}", keyFileVariable: 'PRIVATE_KEY', passphraseVariable: '', usernameVariable: 'USERNAME')]) {
            sh "docker run --rm -w /work -v ${PRIVATE_KEY}:/certs/cert.pem -v ${WORKSPACE}/terraform/:/work hashicorp/terraform destroy --auto-approve -var \"access_key=${AWS_KEY}\" -var \"secret_key=${AWS_SECRET}\"  -var \"private_key_name=${params.Private_Key_Name}\" -var \"private_key=/certs/cert.pem\"  "
          }
        }
      }
    }
